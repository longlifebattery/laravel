...
class HomeController extends BaseController {
	...
	*/
	...
	public function showWelcome()
	{
        ...
        Schema::dropIfExist('art');
        */

        // CHALLENGE VAN H4
        /*
        Schema::create('bestOf', function($newtable)
        {
            $newtable->increments('id');
            $newtable->string('name');
            $newtable->string('artist');
            $newtable->string('year');
        });

        Schema::table('bestOf', function($newcolumn)
        {
            $newcolumn->text('description');
        });

        Schema::dropIfExists('bestOf');
        */

        $theLandmarks = array("St. Marks", "Brooklyn Heights", "Central Park", "Times Squere");
